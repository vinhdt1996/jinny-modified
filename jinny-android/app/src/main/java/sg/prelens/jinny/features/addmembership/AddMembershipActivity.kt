...
//            this.startActivity<BarcodeActivity>()
            if (position < 0 && position > merchantAdapter?.currentList!!.size - 1) {
                return@setOnItemClickListener
            } else {
                this.startActivity<BarcodeActivity>("id" to merchantAdapter?.currentList?.get(position)?.id,
                        "thumb" to merchantAdapter?.currentList?.get(position)?.logo?.url?.thumb,
                        "name" to merchantAdapter?.currentList?.get(position)?.name)
            }
        }
